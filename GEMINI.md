...
- **내 작성 글 목록:** 내가 쓴 글을 비동기로 로딩하고 리스트에서 즉시 수정/삭제 가능.
- **내 채팅 관리:** 현재 진행 중인 모든 채팅방 목록 확인.
...
- **Read Receipt (Seen):** 채팅방에서 상대방이 메시지를 확인하면, 해당 메시지 하단에 **'읽음'** 표시가 실시간으로 나타나도록 구현.
...
- **Status:** 현재 Placeholder로 구현되어 있으며, 추후 개발 예정.
- **Plan:** 일반적인 텍스트 및 이미지 게시판 형태 (댓글 없음, 알림 기반).
