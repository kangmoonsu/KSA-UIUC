...
export function FleaNewPage() {
    ...
    const [category, setCategory] = useState<"SELL" | "BUY">("SELL")

    // Initial item state
    const [items, setItems] = useState<FleaItemState[]>([
        { name: "", price: "", productLink: "", description: "", imageUrls: [], pendingImages: [] }
    ])
    ...
    const handleAddItem = () => {
        setItems([...items, { name: "", price: "", productLink: "", description: "", imageUrls: [], pendingImages: [] }])
    }
    ...
    const handleItemChange = (index: number, field: keyof FleaItemState, value: any) => {
        ...
        try {
            ...
            const processedItems = await Promise.all(items.map(async (item) => {
                ...
                if (item.pendingImages.length > 0) {
                    const uploadedUrls = await uploadImagesMutation.mutateAsync(item.pendingImages);
    ...
    return (
        <div className="container max-w-3xl mx-auto py-10 px-4">
            <h1 className="text-3xl font-bold mb-8 text-navy">중고물품 등록</h1>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <div className="space-y-4 p-6 bg-white rounded-xl border">
                    ...
                    <div className="space-y-2">
                        <Label>거래 유형</Label>
                        <RadioGroup value={category} onValueChange={(val: "SELL" | "BUY") => setCategory(val)} className="flex gap-4">
                            <div className="flex items-center space-x-2">
                                <RadioGroupItem value="SELL" id="sell" />
                                <Label htmlFor="sell" className="cursor-pointer font-normal">판매</Label>
                            </div>
                            <div className="flex items-center space-x-2">
                                <RadioGroupItem value="BUY" id="buy" />
                                <Label htmlFor="buy" className="cursor-pointer font-normal">구매</Label>
                            </div>
                        </RadioGroup>
                    </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <Input
                            id="location"
                            placeholder="예: 309 Green St / 학교 앞"
                            value={location}
                    ...
                    {items.map((item, index) => (
                        <div key={index} className="p-6 bg-white rounded-xl border relative space-y-4 group">
                            ...
                            <div className="space-y-2">
                                ...
                                />
                                <p className="text-xs text-muted-foreground">원래 상품의 정보를 보여주기 위해 링크를 첨부해주세요.</p>
                            </div>
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex gap-4 items-start">
                                    ...
                                    {item.pendingImages.map((file, imgIndex) => (
                                        <div key={imgIndex} className="relative w-20 h-20 border rounded-lg overflow-hidden shrink-0">
                                            <img
                ...
                <div className="flex justify-end gap-2 pt-4 border-t">
                    ...
                    <Button type="submit" size="lg" disabled={createPostMutation.isPending || uploadImagesMutation.isPending}>
                        {(createPostMutation.isPending || uploadImagesMutation.isPending) ? "등록 중..." : "게시글 등록 완료"}
                    </Button>
                </div>
            </form>
        </div>
