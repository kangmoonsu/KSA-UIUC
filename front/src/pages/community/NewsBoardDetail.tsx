...
import { useNewsPost, useDeleteNewsPost } from "@/lib/api/news"
...
export function NewsBoardDetail() {
    const { id } = useParams<{ id: string }>()
    const navigate = useNavigate()
    const { user } = useAuth()
    const { data: post, status } = useNewsPost(id!)
    const { mutate: deletePost } = useDeleteNewsPost()
    ...
    if (status === "error" || !post) return <div className="container max-w-4xl mx-auto py-20 text-center text-red-500">소식을 찾을 수 없습니다.</div>
                ...
                <CardHeader className="border-b bg-slate-50/50 pb-6">
                    <div className="flex justify-between items-start">
                        <div className="space-y-2">
                            <span className="text-orange-500 font-bold text-sm tracking-wider">KSA NEWS</span>
                        ...
                        {(isAuthor || isAdmin) && (
                            <div className="flex gap-2">
                                <Button variant="outline" size="sm" className="gap-2" asChild>
                                    <Link to={`/community/news/${id}/edit`}>
